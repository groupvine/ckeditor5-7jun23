...
function checkForMetaImg($doc:any, elem:any, options?:{ewOnly?:boolean}) {
    if (!options) { options = {}; }
...
export function convertMetaImgsToView(data:string, options?:any) : string {
    if (!options) { options = {}; }
    
    let newData = '<div>' + data + '</div>';   // put in overall wrapper
    let $doc = $(newData);

    // Check <figure>s first, so we replace entire figure element
    // if the embedded img is a GV metaimg
    $doc.find('figure').each( (i:number, elem:any) => {
        checkForMetaImg($doc, elem);
    });

    // Then check individual images (i.e., include those that aren't
    // wrapped in <figure>s
    $doc.find('img').each( (i:number, elem:any) => {
        checkForMetaImg($doc, elem);
    });

    newData = $doc.html();  // Render result within wrapper

    if (options.debugLevel > 3) {
        console.log("convertMetaImgsToView: convert from/to", data, newData);
    }

    return newData;
}

export function anyMetaImgEWs(data:string) {
