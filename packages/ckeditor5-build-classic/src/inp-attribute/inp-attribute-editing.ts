...
import { type Item, type DowncastWriter } from 'ckeditor5/src/engine';
...
export default class InputAttributeEditing extends Plugin {
    ...
    _defineConverters() {
        ...
        let inputAttTypes:Array<any>|null = null;
        let typesDict:{ [index: string]: any } = {};

        function setInputTypes() {
            inputAttTypes = editor.config.get('inputAttribute.types') as Array<any>;

            for (let i = 0; i < inputAttTypes.length; i++) {
                let abbrev = inputAttTypes[i].abbrev as string;
                typesDict[abbrev] = inputAttTypes[i];
            }
        }

        conversion.for( 'upcast' ).elementToElement( {
            // be sure it converts ahead of, e.g., outside wrapper figures or whatever
        ...
        function createInputAttributeView( modelItem:Item, viewWriter:DowncastWriter, options:any ) {
            if (!options) { options = {}; }
            if (inputAttTypes == null) { setInputTypes(); }

            const attType = modelItem.getAttribute( 'type' ) as string;
